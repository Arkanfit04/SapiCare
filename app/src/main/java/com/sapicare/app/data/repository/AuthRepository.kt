...
class AuthRepository @Inject constructor(
    ...
    // Login Google — belum pilih role, return null role dulu
    ...
    suspend fun loginWithGoogle(idToken: String): Result<Pair<String, Boolean>> {
        return try {
            ...
            // Cek apakah sudah punya akun tersimpan
            val existingAccounts = accountManager.savedAccountsFlow.let { flow ->
                var list = emptyList<SavedAccount>()
    ...
    suspend fun saveRoleAccount(role: UserRole): Result<UserSession> {
        return try {
            ...
            Result.success(
                UserSession(uid = uid, username = username, email = email, role = role)
            )
        } catch (e: Exception) {
